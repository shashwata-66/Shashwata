  ...
  useEffect(() => {
    ...
    if (!deleting) {
      if (charIndex < current.length) {
        // Still typing — reveal one more char
        timeout = setTimeout(() => {
      ...
      if (charIndex > 0) {
        // Still deleting — remove one char
        timeout = setTimeout(() => {
          setDisplayed(current.slice(0, charIndex - 1));
          setCharIndex((c) => c - 1);
        }, speed / 2);
...
function Particle({ darkMode }) {
  ...
  return (
    <div
      ...
      style={{
        ...style,
        background: darkMode ? "#a78bfa" : "#7c3aed",
      }}
    />
  );
}
...
export default function Hero({ darkMode }) {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-28 w-full grid md:grid-cols-2 gap-12 items-center">
        {/* Left — Text */}
        <div className="flex flex-col gap-6">
          ...
          >
            ...
            <span className="text-xl font-bold text-purple-400 min-w-[220px]">
              {typed}
              <span className="inline-block w-0.5 h-5 ml-0.5 bg-purple-400 animate-pulse align-middle" />
            </span>
          </motion.div>
          ...
          >
            ...
            <motion.a
              href="/Resume.pdf"
              download="Shashwata_Barman_Resume.pdf"
              target="_blank"
              ...
              className={`px-6 py-3 rounded-xl font-semibold text-sm flex items-center gap-2 border transition-all duration-300 ${
                darkMode
                  ? "border-purple-500/30 text-purple-300 hover:bg-purple-500/10 hover:border-purple-400"
                  : "border-purple-300 text-purple-700 hover:bg-purple-50"
              }`}
            >
              Download Resume <FiExternalLink size={14} />
            </motion.a>
          </motion.div>
            ...
            {socialLinks.map((s, i) => (
              <motion.a
                ...
                className={`p-2.5 rounded-xl border transition-all duration-200 ${
                  darkMode
                    ? "border-purple-500/20 text-gray-400 hover:text-purple-300 hover:border-purple-500/40 hover:bg-purple-500/10"
        ...
        >
          <div className="relative">
            ...
            {/* Photo container */}
            <div
              className={`relative w-72 h-72 rounded-full overflow-hidden border-4 ${
                darkMode ? "border-[#0a0a0f]" : "border-white"
              }`}
            >
              <img
                src={heroImage}
                alt="Shashwata Barman"
            ...
            <motion.div
              ...
              className={`absolute -bottom-3 -left-6 px-4 py-2 rounded-2xl text-xs font-semibold shadow-xl border ${
                darkMode
                  ? "bg-[#13111f] border-purple-500/20 text-purple-300"
      ...
      >
        <span className={`text-xs tracking-widest uppercase ${darkMode ? "text-gray-600" : "text-gray-400"}`}>
          scroll
        </span>
        <motion.div
          animate={{ y: [0, 8, 0] }}
