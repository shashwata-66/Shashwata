...
function StatCard({ stat, darkMode }) {
  ...
  return (
    ...
    >
      <div className={`flex justify-center mb-2 ${darkMode ? "text-purple-400" : "text-purple-600"}`}>
        {stat.icon}
      </div>
      <div className={`text-2xl font-black mb-1 tabular-nums ${darkMode ? "text-white" : "text-gray-900"}`}>
        {count}{stat.suffix}
      </div>
      <div className={`text-xs font-medium ${darkMode ? "text-gray-500" : "text-gray-400"}`}>
        {stat.label}
      </div>
      {/* Subtle "click me" hint */}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="absolute top-1/2 -translate-y-1/2 -right-40 w-[500px] h-[500px] rounded-full opacity-5 blur-[100px]"
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <motion.div {...fadeUp(0)} className="mb-16">
          ...
          <h2 className={`text-4xl md:text-5xl font-black tracking-tight ${darkMode ? "text-white" : "text-gray-900"}`}>
            About{" "}
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="flex flex-col gap-5">
            {cards.map((card, i) => (
              <motion.div
                ...
                className={`p-5 rounded-2xl border transition-all duration-300 group ${
                  darkMode
                    ? "bg-[#0e0e1a] border-purple-500/10 hover:border-purple-500/25 hover:bg-[#11111f]"
                    : "bg-white border-gray-100 hover:border-purple-200 shadow-sm hover:shadow-md"
                }`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`mt-0.5 p-2.5 rounded-xl shrink-0 transition-colors duration-300 ${
                      darkMode
            ...
            <motion.div {...fadeUp(0.55)} className="grid grid-cols-2 gap-4 mt-1">
              {stats.map((stat, i) => (
                <StatCard key={i} stat={stat} darkMode={darkMode} />
              ))}
            </motion.div>
          </div>
          ...
          >
            ...
            <div className={`p-6 rounded-2xl border ${
              darkMode ? "bg-[#0e0e1a] border-purple-500/10" : "bg-white border-gray-100 shadow-sm"
            }`}>
              <p className={`text-xs font-bold tracking-widest uppercase mb-4 ${darkMode ? "text-purple-400" : "text-purple-600"}`}>
                ⚡ Quick Facts
              </p>
            ...
            <motion.div
              ...
              className={`p-5 rounded-2xl border text-sm leading-relaxed ${
                darkMode
                  ? "bg-gradient-to-br from-purple-500/10 to-violet-500/5 border-purple-500/15 text-gray-400"
