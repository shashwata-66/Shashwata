  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center">
        ...
        {index < experiences.length - 1 && (
          <motion.div
            ...
            className="w-px mt-3 flex-1"
            style={{ background: `linear-gradient(to bottom, ${exp.accentColor}40, transparent)` }}
          />
        )}
      </div>
      ...
      <motion.div
        ...
        className={`flex-1 mb-12 p-6 rounded-2xl border transition-all duration-300 group ${
          darkMode
      ...
      >
        {/* Top accent line */}
        <div
          className="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl opacity-0 group-hover:opacity-60 transition-opacity duration-300"
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
          <div>
            {/* Type badge */}
            <span
              className="inline-block px-3 py-0.5 rounded-full text-xs font-bold mb-2 border"
              style={{
            ...
            <p className={`text-sm font-semibold mt-0.5 ${darkMode ? "text-purple-400" : "text-purple-600"}`}>
              {exp.company}
            </p>
          </div>
        ...
        <ul className="flex flex-col gap-3">
          {exp.points.map((point, i) => (
            ...
            >
              ...
              />
              <span className={`text-sm leading-relaxed ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
                {point}
              </span>
            </motion.li>
          ))}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="absolute top-1/2 -translate-y-1/2 -left-40 w-[500px] h-[500px] rounded-full opacity-[0.04] blur-[100px]"
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6">
        ...
        <div className="relative">
          {experiences.map((exp, i) => (
            <TimelineItem key={i} exp={exp} darkMode={darkMode} index={i} />
          ))}
        </div>
      </div>
    </section>
