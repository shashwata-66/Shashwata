  ...
  return (
    <motion.div
      ...
      className={`group relative flex flex-col items-center justify-center gap-3 p-5 rounded-2xl border transition-all duration-300 cursor-default ${
        darkMode ? "bg-[#0e0e1a] border-white/5 hover:border-purple-500/40 hover:bg-[#13101f]"
    ...
    >
      ...
      <div className="text-4xl leading-none transition-transform duration-300 group-hover:scale-110"
        style={{ color: iconColor, filter: `drop-shadow(0 0 8px ${iconColor}55)` }}>
        {skill.icon}
      </div>
      <span className={`text-[11px] font-bold tracking-wide text-center leading-tight transition-colors duration-200 ${
...
export default function Skills({ darkMode }) {
  ...
  // Grid cols for skill pills: more cols on desktop
  const skillGridCols = isDesktop ? "repeat(6, 1fr)" : "repeat(3, 1fr)";

  return (
    <section id="skills" className={`relative py-28 overflow-hidden ${darkMode ? "bg-[#0a0a0f]" : "bg-gray-50"}`}>
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-20 -left-20 w-[400px] h-[400px] rounded-full opacity-[0.04] blur-[100px]" style={{ background: "radial-gradient(circle, #7c3aed, transparent)" }} />
        <div className="absolute -bottom-20 -right-20 w-[400px] h-[400px] rounded-full opacity-[0.04] blur-[100px]" style={{ background: "radial-gradient(circle, #a855f7, transparent)" }} />
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        <div ref={headingRef} className="mb-14">
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={isHeadingInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay: 0.1 }}
            ...
            My <span className="bg-gradient-to-r from-purple-400 to-fuchsia-400 bg-clip-text text-transparent">Skills</span>
          </motion.h2>
          ...
          {categories.map((cat, i) => (
            <button key={cat.label} onClick={() => setActiveTab(i)}
              className={`relative px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center gap-2 ${
                activeTab === i ? "text-white" : darkMode ? "text-gray-500 hover:text-gray-300" : "text-gray-400 hover:text-gray-700"
              }`}
            >
              {activeTab === i && (
                <motion.div layoutId="activeTab" className="absolute inset-0 rounded-xl bg-gradient-to-r from-purple-600 to-violet-600"
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}
          className={`relative rounded-2xl border p-6 min-h-[200px] ${
            darkMode ? "bg-[#0b0b14] border-purple-500/10" : "bg-white border-gray-100 shadow-sm"
          }`}
        >
          <div className="flex items-center gap-3 mb-6">
            <span className="text-2xl">{activeCat.emoji}</span>
            <h3 className={`text-sm font-black tracking-widest uppercase ${darkMode ? "text-purple-400" : "text-purple-600"}`}>{activeCat.label}</h3>
            <div className={`flex-1 h-px ${darkMode ? "bg-white/5" : "bg-gray-100"}`} />
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${darkMode ? "bg-purple-500/10 text-purple-400" : "bg-purple-50 text-purple-600"}`}>
              {activeCat.skills.length} skills
            </span>
          </div>
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} style={{ display: "grid", gridTemplateColumns: skillGridCols, gap: "0.75rem" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="mt-8">
          <p className={`text-xs font-semibold tracking-widest uppercase mb-4 ${darkMode ? "text-gray-600" : "text-gray-400"}`}>All technologies at a glance</p>
          <div className="flex flex-wrap gap-2">
            {categories.flatMap(cat => cat.skills.map(skill => (
              <motion.span key={skill.name} whileHover={{ scale: 1.08, y: -2 }}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200 cursor-default ${
                  darkMode ? "bg-[#0e0e1a] border-white/5 text-gray-400 hover:border-purple-500/30 hover:text-gray-200"
                           : "bg-white border-gray-100 text-gray-500 hover:border-purple-200 shadow-sm"
                }`}
              >
                <span style={{ color: getIconColor(skill, darkMode), fontSize: "0.85rem" }} className="flex items-center">{skill.icon}</span>
                {skill.name}
              </motion.span>
            )))}
          </div>
